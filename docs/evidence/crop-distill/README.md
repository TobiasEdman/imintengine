...
| split verifier attempt 12 | failed closed after both 2,074-file live-cohort checks and the historical frozen split passed: an unbounded local `kubectl` read delayed the watchdog beyond its 180-second lease before terminal publication; all split mounts were read-only and no source or frozen artifact was mutated | Job UID `f5cb9d15-4c84-4e6d-b940-ce4e10928dbe`; Pod UID `678af950-8ea5-4245-b2f8-63441bbeaad0`; failure-record SHA-256 `ec147f530be4403c62b4f589f9c22976441684783953f043ddcd239aeb69b38f`; exit 1, zero restarts |
| split verifier attempt 13 | failed closed after the first 2,074-file live-cohort check: the complete local operator process group was suspended for 965.837 seconds, so the reviewed whole-scan guard refused to renew its expired lease; all split/source/frozen-artifact mounts were read-only and no terminal record was published | Job UID `cfabbdbb-5fc8-48d4-8530-b80984e4a318`; Pod UID `f95fa26e-8908-49f0-b9e1-cd19aa694690`; `refresh-freeze-lease-before-freeze`, exit 1, zero restarts |
...
records the six Pod UIDs, terminal workload-record hashes, normalized/raw Pod
hashes, output hashes, and checksums for every reaper archive file. The full
Pod/log/workload bundles remain restricted outside both Git and the workload
PVC. The receipt deliberately excludes local custody paths and node names; it
is an index, not a substitute for strict offline bundle verification.
