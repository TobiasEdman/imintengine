...
class LSTMClassifier(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, cnn_out_dim, hidden_dim, num_layers, im_height, im_width, bidir=False, two_branches=False):
        ...
        if config.USE_POS_ENC:
            lstm_in_dim += config.POS_ENC_DIM
    ...
    def forward(self, x, seq_lens=None, pos_enc=None):
        ...
        # Optional: Add positional encoding to input tensor
        if pos_enc is not None:
            assert not self.two_branches  # Positional encoding not implemented for two branches yet
            #pos_enc = self.fc_pos_enc(pos_enc)
            #pos_enc = nn.ReLU()(pos_enc)
            if True:  # Concatenate positional encoding to input tensor
                r_in = torch.cat((r_in, pos_enc), dim=2)
            else:  # Add positional encoding to input tensor
                r_in += pos_enc
