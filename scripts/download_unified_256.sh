...
#
# The server (k8s/dataset-256-download.yaml) exposes a basic-auth-gated
# nginx listing of /unified_v2/ over TLS. This mirrors the whole
# directory, resuming any partial/failed files (-c) and retrying.
#
# Usage:
#   DL_USER=<user> DL_PASS='<password>' ./download_unified_256.sh [DEST_DIR]
#
...
: "${DL_USER:?set DL_USER (basic-auth user)}"
: "${DL_PASS:?set DL_PASS (basic-auth password)}"
AUTH=(--user="$DL_USER" --password="$DL_PASS")

mkdir -p "$DEST"
echo "Fetching machine-readable schema -> $DEST/metadata.json"
wget -q "${AUTH[@]}" -O "$DEST/metadata.json" "$META_URL" || echo "  (metadata fetch skipped)"
